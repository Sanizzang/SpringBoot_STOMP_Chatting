...
import com.example.chatting.service.FileService;
...
public class S3FileService implements FileService {
    ...
    public FileUploadDto uploadFile(MultipartFile file, String transaction, String roomId) {
        try{
            ...
            // S3 파일을 저장할 경로인 key를 만든다.
            String key = roomId+"/"+transaction+"/"+filename;

            // 매개변수로 넘어온 multipartFile 을 File 객체로 변환한다.
            // 이렇게 변환된 파일은 S3에 업로드된다.
    ...
    // S3 버킷에서 특정 경로의 모든 파일 삭제
    @Override
    public void deleteFileDir(String path) {
        // amazonS3.listObjects(bucket, path): path 경로에 있는 모든 객체 나열
        // .getObjectSummaries(): 객체의 리스트를 가져옴
        for (S3ObjectSummary summary : amazonS3.listObjects(bucket, path).getObjectSummaries()) {
            // summary.getKey(): 객체의 키를 가져옴
